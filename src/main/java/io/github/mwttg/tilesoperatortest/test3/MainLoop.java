...
public class MainLoop {
  ...
  public MainLoop(final Configuration configuration) throws IOException {
    // create OpenGL ids
    final var animation = FileReader.createFrom("files/test3/animation.json", Animation.class);
    this.vaoId =
        VertexArrayObject.create(animation.tilesGeometry(), animation.textureCoordinates());
    this.textureId = TextureLoader.createFrom("files/test3/animation.png");
    this.animationSteps = List.of(new AnimationStep(0, 6), new AnimationStep(6, 6), new AnimationStep(12, 6),
